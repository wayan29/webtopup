...
Task 13 only. No changes were made to `Sliders.tsx`, Task 14 UI, upload API behavior, server code, packages, services, or production systems.
...
- Added `restrictSelectionTo` while keeping every folder tab browsable; nonmatching folder assets cannot be selected or confirmed. Existing `ImagePickerField` callers retain their folder restriction by default, with an explicit override available.
