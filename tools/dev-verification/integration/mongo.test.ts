...
test('identifier readiness indexes remain disposable-only and are not created by API startup', async () => {
  const main = await fs.readFile(path.join(root, 'rust-api/src/main.rs'), 'utf8');
  assert.equal(main.includes('apply_identifier_indexes'), false);
  const service = await fs.readFile(path.join(root, 'rust-api/src/services/identifier_integrity.rs'), 'utf8');
  assert.match(service, /webtopup_task14_dev/u);
  assert.match(service, /uniq_transactions_reference_id/u);
});
