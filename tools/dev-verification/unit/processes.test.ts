...
test('seller order readiness apply plan is disposable-only and scoped to the debug binary', async () => {
  const root = '/repo';
  const allowed = makeSellerOrderReadinessApplyPlan({
    root,
    stateDir: '/repo/.dev-verification',
    databaseName: 'webtopup_task14_dev',
    mongoUri: 'mongodb://127.0.0.1:27018/webtopup_task14_dev?replicaSet=rs0&directConnection=true',
    executable: '/repo/rust-api/target/debug/seller_order_readiness',
  });
  assert.deepEqual(allowed, {
    command: '/repo/rust-api/target/debug/seller_order_readiness',
    args: ['--apply', '--json'],
    cwd: '/repo/rust-api',
    envKeys: ['MONGO_URI', 'MONGO_DB'],
  });
  for (const databaseName of ['webtopup', 'webtopup_task14', 'webtopup_task14_dev_backup', '']) {
    assert.throws(() => makeSellerOrderReadinessApplyPlan({
      root,
      stateDir: '/repo/.dev-verification',
      databaseName,
      mongoUri: `mongodb://127.0.0.1:27018/${databaseName}?replicaSet=rs0&directConnection=true`,
  ...
  assert.throws(() => makeSellerOrderReadinessApplyPlan({
    ...
    databaseName: 'webtopup_task14_dev',
    mongoUri: 'mongodb://127.0.0.1:27018/webtopup_task14_dev?replicaSet=rs0&directConnection=true',
    executable: '/tmp/seller_order_readiness',
  }), /expected seller readiness binary/);
  const source = await fs.readFile(path.resolve(import.meta.dirname, '..', 'processes.ts'), 'utf8');
  assert.match(source, /await prepareDisposableSellerOrderIndexes\(config\)/);
  assert.doesNotMatch(source, /allow-protected-database/);
});
