...
import { canActivateSlider, formatArchivedMeta, sliderPositionLabel, sliderStatusLabel, type SliderView } from '../../lib/sliderPresentation';
import {
    DndContext,
    KeyboardSensor,
    PointerSensor,
                ...
                <div className="flex items-center gap-2">
                    {!archived ? (
                        ...
                        <button
                            ...
                            disabled={busy || !mutationEnabled}
                            aria-label={`Pulihkan slider ${slider.name}`}
                            title="Pulihkan slider sebagai draft"
        ...
        <article className="rounded-2xl border ui-border ui-panel-muted p-3 shadow-sm">
            ...
            <div className="mt-3 grid gap-2 text-sm">
                <div className="flex items-center justify-between rounded-lg border ui-border px-3 py-2">
                    ...
                    <span className="font-semibold ui-text">{sliderPositionLabel({ sortOrder: slider.sortOrder, total: positionTotal, filtered: positionFiltered, archived })}</span>
                </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
                {!archived && canReorder ? (
                    <>
                        <button type="button" onClick={() => onMove(index, -1)} disabled={busy || index === 0} aria-label={`Naikkan slider ${slider.name}`} className="inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-semibold ui-muted-action disabled:opacity-40">
                            <ArrowUp className="h-3.5 w-3.5" aria-hidden="true" /> Naikkan
                        </button>
                        <button type="button" onClick={() => onMove(index, 1)} disabled={busy || index === total - 1} aria-label={`Turunkan slider ${slider.name}`} className="inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-semibold ui-muted-action disabled:opacity-40">
                            <ArrowDown className="h-3.5 w-3.5" aria-hidden="true" /> Turunkan
                        </button>
                    </>
                ) : null}
                {!archived ? (
                    ...
                    <button type="button" onClick={() => onRestore(slider)} disabled={busy || !mutationEnabled} aria-label={`Pulihkan slider ${slider.name}`} className="rounded-lg border px-3 py-2 text-xs font-semibold ui-success-chip disabled:opacity-50">Pulihkan</button>
                )}
            </div>
        </article>
    );
...
export default function Sliders() {
    ...
    const confirmRestore = async () => {
        ...
        await performMutation(intent, `Slider "${restoreTarget.name}" berhasil dipulihkan sebagai draft`, 'restore');
    };
        ...
        <div className="space-y-5" aria-busy={loading || saving || sorting ? 'true' : 'false'}>
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="inline-flex rounded-xl border ui-border ui-panel-muted p-1" role="tablist" aria-label="Tampilan slider">
                    <button id="slider-current-tab" type="button" role="tab" aria-controls="slider-current-panel" aria-selected={view === 'current'} onClick={() => changeView('current')} className={`rounded-lg px-4 py-2 text-sm font-semibold ${view === 'current' ? 'ui-accent-solid' : 'ui-muted-action'}`}>{'Aktif & Draft'}</button>
                    <button id="slider-archive-tab" type="button" role="tab" aria-controls="slider-archive-panel" aria-selected={view === 'archive'} onClick={() => changeView('archive')} className={`rounded-lg px-4 py-2 text-sm font-semibold ${view === 'archive' ? 'ui-accent-solid' : 'ui-muted-action'}`}>Arsip</button>
                </div>
            ...
            <div id={view === 'current' ? 'slider-current-panel' : 'slider-archive-panel'} role="tabpanel" aria-labelledby={view === 'current' ? 'slider-current-tab' : 'slider-archive-tab'} className="space-y-5">
            ...
            <div className="rounded-xl border ui-border ui-panel-muted p-4 space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm ui-text-muted">
                    ...
                    {view === 'current' ? (!canReorder
                        ...
                        : <span className="rounded-full border ui-border px-3 py-1 text-xs">Drag atau tombol Naikkan/Turunkan aktif</span>) : null}
                </div>
            </div>
            ...
            {!loading && snapshot && filteredItems.length === 0 ? (
                <div className="space-y-3 rounded-xl border ui-border ui-panel-muted px-4 py-10 text-center ui-text-muted">
                    {items.length === 0 ? (
                        view === 'current' ? (
                            <>
                                <p>Belum ada slider aktif atau draft.</p>
                                {canAdd ? (
                                    <div>
                                        <button type="button" onClick={(event) => openAddModal(event)} className="inline-flex items-center gap-2 rounded-xl ui-accent-solid px-4 py-2.5 text-sm font-semibold">
                                            <Plus className="h-4 w-4" aria-hidden="true" /> Tambah slider pertama
                                        </button>
                                    </div>
                                ) : null}
                            </>
                        ) : 'Belum ada slider yang diarsipkan.'
                    ) : (
                        <>
                            <p>Tidak ada slider yang cocok dengan filter.</p>
                            <div>
                                <button type="button" onClick={resetFilters} className="inline-flex items-center gap-2 rounded-lg border px-4 py-2 text-sm font-semibold ui-muted-action"><RotateCcw className="h-4 w-4" aria-hidden="true" /> Hapus filter</button>
                            </div>
                        </>
                    )}
                </div>
            ...
            <AccessibleDialog open={dialogKind === 'form'} titleId="slider-form-title" descriptionId="slider-form-description" dialogRef={formDialogRef} initialFocusRef={formInitialFocusRef} returnFocusRef={formReturnFocusRef} busy={saving} onClose={resetDialog}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto p-4">
                    ...
                    <div className="space-y-2">
                        <label className={`flex items-center gap-2 rounded-lg border ui-border ui-panel-muted p-3 text-sm ui-text ${!canSelectActive && !form.status ? 'opacity-70' : ''}`}><input type="checkbox" checked={form.status} disabled={!canSelectActive && !form.status} onChange={(event) => setForm({ ...form, status: event.target.checked })} className="h-4 w-4" /> Publikasikan sebagai slider aktif</label>
                        {activeCapacityMessage ? <p className="text-xs ui-warning-chip">{activeCapacityMessage}</p> : null}
                    </div>
