...
import IrsSellerSettings from './IrsSellerSettings';

type ActiveChannel = 'api' | 'irs';
type ActiveTab = 'settings' | 'mapping' | 'webhook';
...
export default function DigiflazzSellerSettings() {
    const stepUp = useStepUpOrchestration();
    const [activeChannel, setActiveChannel] = useState<ActiveChannel>('api');
    ...
    const [mappingEditor, setMappingEditor] = useState<MappingEditorState>({
        pulsaCode: '',
        sellerMarginFlat: '',
        isActive: true,
        syncNow: true
    ...
    }, [activeTab, mappingPage, mappingFilter]);
    ...
    const derivedEndpointPreview = useMemo(() => {
        if (settings.prepaidEndpointUrl) {
            return settings.prepaidEndpointUrl;
        }
  ...
  -d '{"limit":${schedulerConfig.exampleLimit}}'`;
    ...
    const fetchSettings = async () => {
        try {
            ...
            setSettings({
                ...
                retryQueueHealth: {
                    ...defaultRetryQueueHealth,
                    ...(data.retryQueueHealth || {})
                }
            });
        } catch (error) {
    ...
    const fetchMappings = async (searchValue?: string) => {
        try {
            ...
            const response = await apiV2
                .get(`/digiflazz-seller/mappings?${params.toString()}`);
    ...
    const fetchSchedulerConfig = async () => {
        try {
            const response = await apiV2
                .get('/digiflazz-seller/orders/process-callback-retries/scheduler/config');
            setSchedulerConfig({
    ...
    const handleSaveMapping = async (event: React.FormEvent) => {
        ...
        try {
            ...
            await apiV2
                .post('/digiflazz-seller/mappings', mappingPayload);
    ...
    const handleDeleteMapping = async () => {
        ...
        if (!window.confirm('Hapus mapping Digiflazz Seller untuk produk ini?')) {
            return;
        }
        ...
        try {
            ...
            await apiV2
                .delete(`/digiflazz-seller/mappings/${selectedMappingId}`);
        ...
        <div className="space-y-5">
            <div className="ui-panel-muted flex flex-wrap gap-2 rounded-xl border ui-border p-4">
                <button
                    onClick={() => {
                        fetchSettings();
                        if (activeTab === 'mapping') fetchMappings();
                        if (activeTab === 'webhook') {
                            fetchLogs();
                            fetchOrders();
                        }
                    }}
                    className="inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold ui-muted-action hover:border-[var(--ui-accent)]"
                >
            ...
            <div className="rounded-2xl border ui-border ui-panel-muted p-3">
                <div className="grid gap-2 sm:grid-cols-2">
                    {([
                        { key: 'api', label: 'API', description: 'Endpoint dan mapping Digiflazz Seller standar' },
                        { key: 'irs', label: 'IRS', description: 'Channel IRS memakai mapping produk yang sama' }
                        ...
                        return (
                            <button
                                ...
                                className={`rounded-xl border px-4 py-3 text-left transition-colors ${active ? 'ui-accent-chip' : 'ui-muted-action'}`}
                            >
                                <span className="block text-sm font-black">{channel.label}</span>
                                <span className="mt-1 block text-xs ui-text-muted">{channel.description}</span>
                            </button>
                        );
                    })}
                </div>
            ...
            <div className="flex gap-2 border-b ui-border">
                {[
                    { key: 'settings', label: 'Settings', icon: Server },
                    { key: 'mapping', label: 'Mapping Produk', icon: Zap },
                    { key: 'webhook', label: 'Webhook & Orders', icon: Cable }
                ].map((tab) => {
                    const Icon = tab.icon;
                    ...
                    return (
                        <button
                            ...
                            className={`inline-flex items-center gap-2 border-b-2 px-4 py-2 text-sm font-semibold transition-colors ${active ? 'border-[var(--ui-accent)] ui-accent-text' : 'border-transparent ui-text-muted hover:text-[var(--ui-text)]'}`}
                        >
                ...
                <div className="grid gap-5 xl:grid-cols-[1.4fr_1fr]">
                            ...
                            <label className="space-y-2">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        placeholder={settings.apiKeyMasked || 'Masukkan API key'}
                                ...
                                {settings.apiKeyMasked && (
                                    <p className="text-xs ui-text-muted">API key tersimpan saat ini: {settings.apiKeyMasked}. Kosongkan field jika tidak ingin mengubahnya.</p>
                                )}
                            </label>
                    ...
                    <div className="space-y-4 rounded-2xl border ui-border ui-panel-muted p-5">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="rounded-xl border ui-border ui-panel p-4">
                                <div className="text-xs uppercase tracking-[0.18em] ui-accent-text">Checklist Setup</div>
                                <ul className="mt-2 space-y-2 text-sm ui-text-muted">
                                    <li>1. Daftarkan `username` seller yang sama dengan panel Digiflazz.</li>
                                    <li>2. Masukkan `Public Base URL` agar endpoint prabayar bisa disalin dengan benar.</li>
                                    <li>3. Salin `Report / Callback URL` unik dari panel Digiflazz Seller ke field di sebelah kiri.</li>
                                    <li>4. Pastikan whitelist inbound memuat IP `52.74.250.133`.</li>
                                    <li>5. Jika app di belakang Nginx atau proxy, pastikan hanya proxy tepercaya yang bisa mengakses port aplikasi.</li>
                                    <li>6. Buat minimal satu mapping produk aktif dan sinkronkan ke panel Digiflazz Seller.</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                                                        ...
                                                        {hasMapping ? (
                                                            <div className="space-y-1">
                                                                <div className="ui-text">
                                                                    Rp{currencyFormatter.format(Number(item.mapping?.effectiveMarginFlat ?? settings.sellerMarginFlat))}
                                                                </div>
                                                            ...
                                                            <div className="space-y-1">
                                                                 <div className="ui-text">Rp{currencyFormatter.format(Number(settings.sellerMarginFlat || 0))}</div>
                        ...
                        <div className="rounded-2xl border ui-border ui-panel-muted p-5">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold ui-muted-action hover:border-[var(--ui-accent)]"
                                >
                                ...
                                <div className="rounded-xl border ui-border ui-panel p-4">
                                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                        ...
                                        <button
                                            ...
                                            className="inline-flex items-center justify-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold ui-muted-action"
                                        >
                                            <RefreshCw className="h-3.5 w-3.5" /> Refresh Config
                                        </button>
                                    </div>
                        ...
                        <form onSubmit={handleSaveMapping} className="mt-4 space-y-4">
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div className="text-xs ui-text-muted">
                                    Digiflazz Seller prabayar hanya membawa satu field target `hp`. Harga default mapping memakai `costPrice + margin seller`, jadi perubahan harga publik basic, gold, atau platinum tidak memengaruhi harga seller.
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {selectedMappingId && (
                                        <button
